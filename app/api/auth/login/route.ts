...
import { verifySecret, createSessionToken, SESSION_COOKIE_NAME, SESSION_MAX_AGE } from "@/lib/auth";
...
export async function POST(req: NextRequest) {
  ...
  const token = await createSessionToken({ userId: user._id.toString(), username: user.username });
